...
6. **Render-test** in a throwaway VoidMulti-only staging mod. Zerg-isolated test hook (mod is Terran-only): add a `LarvaTrain` Train entry + a Larva card button in a FREE slot (bottom-right / Row2 Col4) — morph a Larva → the unit. Confirm it renders/animates/attacks (**both** weapons — test air attack vs a flyer).
7. **Custom missile movers don't translate — swap to `MissileDefault`.** If a unit's air/ranged attack "launches a missile that then disappears and deals zero damage," the missile unit uses a campaign-custom `CMoverMissile` that doesn't drive in the multiplayer engine. Fix: on the missile unit, change `<Mover value="<Unit>Missile"/>` → `<Mover value="MissileDefault"/>` (the shared mover that works). Confirmed on Goliath's Hellfire missiles (its ground attack was fine because it's direct-damage, no missile; the Wraith "just worked" because its missiles already used `MissileDefault`). Costs the fancy missile arc, keeps full function.

**Benign warning to ignore:** "Too many CAbil[Stop/Attack/Move] abilities for <unit>" fires for extracted units but is non-functional (the same warning hits untouched units like Battlecruiser; the ability arrays are actually clean — one each).
