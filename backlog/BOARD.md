...
## 🔨 In progress
- [WA-044](./WA-044-transfuse-targeting.md) — "Must target biological units" was the Queen's **native `Transfusion`** (not the rolled `F_Transfusion` I first checked) inheriting the stock bio-gated TargetFilters. Added a clean TargetFilters override. **PR open — re-test the Queen transfusing a Siege Tank** (locally testable).
