...
## ✅ Done 2026-07-16 (PR)
Added `Button/Tooltip/Colossus` to the mod's `GameStrings.txt`, mirroring the Marine (WA-016) / Hydralisk (WA-017) free-upgrade tooltips (train-button tooltip, upgrade called out inline):
> Long-range area-damage strider; walks over cliffs. Armed with Extended Thermal Lance (+2 range, total 9). Can attack ground units only.

Colossus train button face confirmed `Colossus` (`FactoryTrain` in AbilData). Pure GameStrings — locally testable.
...
## Acceptance criteria
- [x] A tooltip visible in-game states the Colossus starts with Extended Thermal Lance / 9 range.
- [x] Wording/placement consistent with the Marine + Hydralisk free-upgrade tooltips.
