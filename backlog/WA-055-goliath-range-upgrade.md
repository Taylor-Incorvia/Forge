...
## ✅ Done 2026-07-20 (PR)
Custom `GoliathRange` catalog upgrade (mirrors `SiegeTankRange`): **+3.5** to `GoliathA`/`GoliathAUpgraded` range, **+1.5** to `GoliathG`/`GoliathGUpgraded` (bumped from +3/+1 in review — "make it spicy") (verified the Goliath's WeaponArray uses all four). Rolls at Factory slot 2 → Armory col 1 (`GoliathRange2`); generic `Range` now `NoneOf Goliath`. **Open decision resolved:** added to the **Range family** (shares the range cap with Range/SiegeTankRange/TempestRange) — consistent with the others; revisit if the shared cap-2 feels tight in playtests.
